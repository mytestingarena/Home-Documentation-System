...
function hds_permanent_log_parse_contractor_fields(array $post): array
{
    ...
    $payment_method = in_array($post['perm_log_payment_method'] ?? '', ['debit', 'cc', 'check'], true)
...
function hds_render_permanent_contractor_fields(
    ...
    echo "</div><br>";
    echo "<label>Debit / CC / Check #:</label> <input type='text' name='perm_log_payment_ref' value=\"$payment_ref\" placeholder='Last 4, check number, etc.'><br><br>";
    echo "</div>";
}
...
function hds_render_permanent_maintenance_log($conn, int $house_id, string $item_type): void
{
    ...
    if ($logs && $logs->num_rows > 0) {
        while ($log = $logs->fetch_assoc()) {
            ...
            echo "<form method='post' class='maintenance-form perm-log-form'>";
            echo "<input type='hidden' name='permanent_log_id' value='$log_id'>";
            echo "<input type='hidden' name='item_type' value='$item_type'>";
    ...
    echo "<form method='post' class='maintenance-form maintenance-log-add perm-log-form'>";
    echo "<input type='hidden' name='item_type' value='$item_type'>";
